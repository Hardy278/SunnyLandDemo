...
TextureManager::TextureManager(SDL_Renderer *renderer) : m_renderer(renderer) {
    if (!m_renderer) {
        throw std::runtime_error("RESOURCEMANAGER::TEXTUREMANAGER::ERROR::SDL_Renderer未初始化, 请检查是否正确初始化SDL");
    }
    // 初始化SDL_Image
...
SDL_Texture *TextureManager::loadTexture(const std::string_view path) {
    ...
    if (it != m_textures.end()) {
        spdlog::warn("RESOURCEMANAGER::TEXTUREMANAGER::WARN::已存在同名纹理, 将使用原纹理");
    ...
    if (!rawTexture) {
        spdlog::error("RESOURCEMANAGER::TEXTUREMANAGER::loadTexture::ERROR::加载纹理失败: {} : {}", path, SDL_GetError());
        return nullptr;
    }
...
SDL_Texture *TextureManager::getTexture(const std::string_view path) {
    ...
    spdlog::warn("RESOURCEMANAGER::TEXTUREMANAGER::getTextureSize::WARN::未找到纹理, 尝试加载: {}", path);
...
glm::vec2 TextureManager::getTextureSize(const std::string_view path) {
    ...
    if (!texture) {
        spdlog::error("RESOURCEMANAGER::TEXTUREMANAGER::getTextureSize::ERROR::未找到纹理 \"{}\"", path);
        return glm::vec2(0, 0);
    }
    glm::vec2 size;
    if (!SDL_GetTextureSize(texture, &size.x, &size.y)) {
        spdlog::error("RESOURCEMANAGER::TEXTUREMANAGER::getTextureSize::ERROR::获取纹理大小失败: {}", SDL_GetError());
        return glm::vec2(0, 0);
    }
    return size;
}
...
void TextureManager::unloadTexture(const std::string_view path) {
    ...
    if (it != m_textures.end()) {
        ...
        spdlog::warn("RESOURCEMANAGER::TEXTUREMANAGER::unloadTexture::WARN::未找到纹理 \"{}\"", path);
    }
}
...
void TextureManager::clearTextures() {
    if (!m_textures.empty()) {
        ...
        spdlog::warn("RESOURCEMANAGER::TEXTUREMANAGER::clearTextures::WARN::纹理列表为空, 无需清理");
    }
}
/// @}
