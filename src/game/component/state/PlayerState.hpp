...
protected:
    PlayerComponent* m_playerComponent = nullptr;   ///< @brief 指向拥有此状态的玩家组件

public:
    PlayerState(PlayerComponent* playerComponent) : m_playerComponent(playerComponent) {}
    ...
    void playAnimation(const std::string& animationName);      ///< @brief 播放指定名称的动画，使用 AnimationComponent 的方法
...
protected:
    ...
    /* update 返回值为下一个状态，如果不需要切换状态，则返回 nullptr */
